  ...
  <div class="payment_handleprice">
    <div class="payment_handleprice-pricewrap">
      ...
      <div class="payment_detailwrap">
        ...
        <div class="payment-box" v-if="!isWalletConfirming && isExpiredExchange">
        ...
        <div class="dattail-lists mt-1" v-if="!isWalletConfirming">
          <div class="dattail-list add-flex j-between mb-1">
            <p>Exchange Rate</p>
            <p>1 {{ merchantReceiveTokenSymbol }} &#65309; {{ userSelectedTokenExchangeRate }} {{ userSelectedTokenSymbol }}<img src="@/assets/images/exchange.svg" alt=""></p>
          </div>
          <div class="dattail-list add-flex j-between mb-1">
            <p>Route</p>
          ...
          <p class="dattail-list_desc">
            Output is estimated. You will receive at least {{ merchantReceiveAmount }} {{ merchantReceiveTokenSymbol }} or the transaction will revert.
          </p>
        </div>
        <div v-else class="payment-status mt-3 mb-3">
          <div>
            <img class="mb-2 spin" src="@/assets/images/loading.svg" alt="waiting">
            <p class="payment-status_title">
              Waiting for Confirmation
            </p>
            <p class="payment-status_desc mb-2">
              Pay {{ userSelectedTokenPaymentAmount }} {{ userSelectedTokenSymbol }} for {{ merchantReceiveAmount }} {{ merchantReceiveTokenSymbol }}
            </p>
          </div>
        </div>
        ...
        <button v-else-if="isWalletConfirming" class="btn __g __l mb-2 inactive">
          Please confirm Wallet
        </button>
        <button v-else class="btn __g __l mb-2" :class="{ inactive: isExpiredExchange }" @click="executePayment()">
          Confirm Wallet
        </button>
        <p class="via">
          via Slash Payment
...
export default {
  ...
  computed: {
    ...
    isSelectedNativeToken() {
      return Object.values(this.NATIVE_TOKEN_SYMBOLS)
        .includes(this.userSelectedTokenSymbol)
    },
    isExpiredExchange() {
      return this.expired
    },
